...
const ChatInterface = () => {
    ...
    const renderMessage = (message) => {
        ...
        if (message.sender === 'ai' && !message.isError) {
            ...
            return (_jsx("div", { className: "markdown-content", children: _jsx(ReactMarkdown, { children: content }) }));
        }
        return message.text;
    };
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: `chat-container ${isCollapsed ? 'collapsed' : ''}`, children: [_jsxs("div", { className: "chat-header", children: [_jsx("h2", { children: "Recombyne Copilot" }), _jsx("button", { className: "collapse-button", onClick: toggleCollapse, "aria-label": isCollapsed ? "Expand chat" : "Collapse chat", children: isCollapsed ? '+' : '×' })] }), _jsxs("div", { className: "messages-container", ref: messagesEndRef, children: [messages.map(message => (_jsx("div", { className: `message ${message.sender === 'user' ? 'user-message' : 'ai-message'} ${message.isError ? 'error-message' : ''}`, children: renderMessage(message) }, message.id))), isProcessing && (_jsx("div", { className: "message ai-message processing", children: _jsx(TextShimmer, { text: "Thinking..." }) }))] }), _jsxs("div", { className: "chat-bottom-container", children: [_jsxs("div", { className: "search-button-container", children: [_jsx("button", { className: "sequence-action-button", onClick: openGenBankSearchPopup, children: "Find Sequences" }), _jsx("button", { className: "sequence-action-button", onClick: handleOptimizeClick, children: "Optimize" })] }), _jsx("form", { className: "input-container", onSubmit: handleSendMessage, children: _jsxs("div", { className: "input-field-wrapper", children: [_jsx("input", { type: "text", value: inputText, onChange: (e) => setInputText(e.target.value), placeholder: "Ask anything", disabled: isProcessing }), _jsx("button", { type: "submit", className: "send-button", disabled: !inputText.trim() || isProcessing, children: "Send" })] }) })] })] }), _jsx(GenBankSearchPopup, { isOpen: isGenBankPopupOpen, onClose: closeGenBankSearchPopup }), isPlasmidSelectorOpen && (_jsx(PlasmidSelectorModal, { isOpen: isPlasmidSelectorOpen, onClose: closePlasmidSelector })), _jsx(SequenceOptimizerModal, { isOpen: isSequenceOptimizerOpen, onClose: () => setIsSequenceOptimizerOpen(false), sequence: currentSequence, onOptimizeComplete: (optimizedSequence, summary) => {
